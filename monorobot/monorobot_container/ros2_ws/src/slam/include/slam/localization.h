...
struct PosedScan
{
    ...
    PosedScan(Symbol x, Pose3 pose, const sensor_msgs::msg::LaserScan& scan, bool updated, bool keep = true)
        : x(x), pose(pose), scan(scan), updated(updated), keep(keep) {}

    PosedScan(Pose3 pose, const sensor_msgs::msg::LaserScan& scan, bool updated, bool keep = false)
...
private:
    ...
    std::optional<Symbol> nearest_neighbor_data_association(const Point3& observed_point);
    std::optional<Symbol> flann_data_association(const Point3& observed_point);
    std::vector<std::pair<Symbol, double>> probabilistic_data_association(const Point3& observed_point);
    void localization();
    ...
    std::vector<std::pair<size_t, size_t>> factor_indices_per_pose;
