...
void Mapping::mapping()
{
...

    {
        ...
        for (PosedScan& posed_scan : *posed_scans)
        {
            ...
            for (size_t i = 0; i < scan.ranges.size(); ++i)
            {
                ...
                // Transform to global
                double gx = pose.x() + lx * cos(pose_theta) - ly * sin(pose_theta);
                double gy = pose.y() + lx * sin(pose_theta) + ly * cos(pose_theta);
    ...
    if(override_active_hit_points)
    {
        ...
        Pose3& pose = last_posed_scan.pose;
